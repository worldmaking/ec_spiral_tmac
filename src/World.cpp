...
void World::reset() {
	...
	for (int i = 0; i < NUM_ORGANISMS; i++) {
		...
		o.thing.alive = 1;
		o.thing.nrg = glm::linearRand(0., 1.);


		o.thing.pos = glm::dvec3(0.5 + glm::linearRand(0., double(DIM - 1)),
			0.5 + glm::linearRand(0., double(DIM - 1)),
			0.5 + glm::linearRand(0., double(DIM - 1)));
		...
		glm::dvec3 euler(glm::linearRand(0.f, (float)M_PI), glm::linearRand(0.f, (float)M_PI), glm::linearRand(0.f, (float)M_PI));
		o.orient = glm::dquat(euler);
		o.dorient = glm::dquat(glm::dvec3(glm::linearRand(-0.1f, 0.1f), glm::linearRand(-0.1f, 0.1f), glm::linearRand(-0.1f, 0.1f)));
		o.turn = glm::dvec3(0.);
		o.vary = glm::linearRand(0.f, 0.4f);// //?
		o.flash = glm::linearRand(0.f, 1.f);// //?
		o.speed = glm::linearRand(0.f, 1.f);// //?
		o.age = 0;
	...
	for (unsigned int i = 0; i < NUM_STALKS; i++) {
		...
		o.color = glm::dvec3(glm::linearRand(0., 1.), 0.1, glm::linearRand(0., 1.));
		o.length = 1.;
		o.numchildren = 0;
		o.parent = 0;
		...
		if (i > (NUM_STALKS/10)) {
			...
			o.thing.pos = glm::dvec3(glm::linearRand(0.f, float(DIM)), glm::linearRand(0.f, float(DIM)), glm::linearRand(0.f, float(DIM)));
			glm::dvec3 euler(glm::linearRand(0.f, (float)M_PI), glm::linearRand(0.f, (float)M_PI), glm::linearRand(0.f, (float)M_PI));
			o.orient = glm::dquat(euler);
	...
	for (int i = 0; i < NUM_PARTICLES; i++) {
		...
		base.pos = glm::vec3(glm::linearRand(0.f, float(DIM)), glm::linearRand(0.f, float(DIM)), glm::linearRand(0.f, float(DIM)));
		o.neighbor = NULL;
